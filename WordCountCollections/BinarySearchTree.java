...
public class BinarySearchTree<T extends Comparable<T>> implements WordCountCollection<T>
{
   ...
   private void traverse(int type, ArrayList<T> list,BinaryNode node){
        if(node!=null){
            if(type==0){
                list.add((T)node.getInfo());
                traverse(0, list, node.getLeft());
                traverse(0, list, node.getRight());
            }else if(type==1){

                traverse(1, list,node.getLeft());
                list.add((T)node.getInfo());
                traverse(1, list,node.getRight());
            }else if(type==2){
                traverse(2, list,node.getLeft());
                traverse(2, list,node.getRight());
                list.add((T)node.getInfo());
            }
        }
    }
